...
router.post(
  ...
  async function (req, res: ProfileResponse<IssueSuggestionOutput>) {
    ...
    const udpatedEpics = await insertIssueToEpicsInDocument(
      issueData,
      projectId,
      oldDevPlan
    );
...
// upsert document
router.post('/addPublish', async function (req, res: ProfileResponse<Project>) {
  const currentUser = res.locals.currentUser;
...
// Get all active isssues for the current user
router.get(
  '/',
  async function (request, response: ProfileResponse<IssueOutput[]>) {
    try {
      ...
      console.error('Error in GET /issues/all', error);
      response.status(500).json({
        success: false,
        errorMsg: (error as string | Error).toString(),
      });
...
// update comment
// TODO: add auth check: comment should be update by its author
router.post(
