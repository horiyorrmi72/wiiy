...
export default function IssueEditor({ issueId, workPlanId, onSuccess }: args) {
  ...
  */
  ...
  const milestoneEpicsTreeData = getMilestoneEpicsTreeData(project);
    ...
    <Spin spinning={isLoading}>
      ...
      >
        <Form.Item
          label="Issue Name"
          name="name"
          rules={[{ required: true, message: 'Please input issue name' }]}
        >
          <TextArea placeholder="Enter name of issue" />
        </Form.Item>
        <Form.Item
          label="Parent Epic"
          name="parentIssueId"
          rules={[{ required: true, message: 'Please select parent epic' }]}
        >
          <TreeSelect
            showSearch
            style={{ width: '100%' }}
        ...
        >
          <Radio.Group value={'auto'}>
            ...
            <Radio value={'preset'}>Pre-select</Radio>
            <Radio value={'backlog'}>Backlog</Radio>
          </Radio.Group>
        </Form.Item>

        <Form.Item label="Sprint" name="workPlanId">
          <Select
            allowClear={true}
            options={sprintData}
...
function getMilestoneEpicsTreeData(project: ProjectOutput) {
  return [
