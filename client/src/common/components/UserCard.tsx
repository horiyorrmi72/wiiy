...
export function UserCard({ user, secondaryInformation }: UserCardProps) {
  return (
    <Space>
      ...
      <Space>
        <Typography.Text>{getDisplayInfo(user, 'username')}</Typography.Text>
        {secondaryInformation && secondaryInformation.length !== 0 && (
          <Typography.Text>
            (
            {secondaryInformation
              .map((info) => getDisplayInfo(user, info))
              .join(', ')}
            )
          </Typography.Text>
        )}
...
export function getDisplayInfo(user: User, info: UserInformationType): string {
  switch (info) {
    case 'username':
    ...
    case 'specialty':
      return getSpecialtyDisplayName(user.specialty);
    default:
      return '';
  }
}
